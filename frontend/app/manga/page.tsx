...
export default function MangaSearchPage() {
  ...
  const { 
    ...
    hasNextPage,
    isError 
  } = useInfiniteSearchManga(filters, 18, 12)

  const { data: filterOptions } = useMangaFilterOptions()
  ...
  const allMangas = useMemo(() => {
    ...
    return Array.from(new Map(items.map((manga) => [manga.id, manga])).values())
  }, [data])

  const totalResults = data?.pages[0]?.meta.total ?? 0

  const updateFilters = useCallback((newFilters: Partial<MangaFilters>) => {
  ...
  const toggleFilter = (type: keyof MangaFilters, value: string) => {
  ...
  const isFilterActive = (type: keyof MangaFilters, value: string) => {
    const current = (filters[type] as string[]) || []
                ...
                return value.map(v => (
                  <Badge key={`${key}-${v}`} variant="secondary" className="gap-2 px-3 py-1.5 bg-primary/10 text-primary border-primary/20 hover:bg-primary/20 transition-colors rounded-full font-medium">
                    ...
                    <X 
                      ...
                      onClick={() => toggleFilter(key as keyof MangaFilters, v)}
                    />
                  </Badge>
                ))
              }
