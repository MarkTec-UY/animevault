...
function normalizeLibraryItem(raw: unknown): LibraryItem | null {
  ...
  return {
    ...
    status: (item.status as LibraryStatus) ?? "planning",
    progress_episodes: toNumber(item.progress_episodes),
    score: item.score == null ? null : toNumber(item.score),
    started_at: safe(item.started_at, null),
    completed_at: safe(item.completed_at, null),
...
async function tryFetch<T>(endpoint: string, fallback: T): Promise<T> {
  try {
    return await apiFetchServer<T>(endpoint, { cache: "no-store" })
  } catch {
    return fallback
  }
}
...
export async function getProfilePayload(
  ...
  // 1. Resolve user by username. Endpoint accepts username or id.
  ...
  const user = normalizeUser(
    userResponse && "user" in (userResponse as object)
      ? (userResponse as { user: unknown }).user
      : userResponse
  )

  if (!user) return null

  // 2. Determine if this is the viewer's own profile.
  ...
  const [libraryRaw, favoritesRaw, notificationsRaw] = await Promise.all([
    tryFetch<{ data?: unknown[] } | unknown[]>(
      `/api/v1/users/${user.id}/library?per_page=200`,
      []
    ),
    tryFetch<{ data?: unknown[] } | unknown[]>(
    ...
    isOwnProfile
      ? tryFetch<{ data?: unknown[] } | unknown[]>(
          `/api/v1/me/notifications?per_page=10`,
          []
        )
